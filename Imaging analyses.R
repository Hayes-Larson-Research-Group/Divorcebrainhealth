...
for (i in 1:length(all_expvars)){
  ...
  if (i==1){all_res_pet <-rbind(temp.mod.crude.summ.df, temp.mod.adj1.summ.df)} else {
    all_res_pet <-rbind(all_res_pet, temp.mod.crude.summ.df, temp.mod.adj1.summ.df)
  }
...
for (i in 1:length(all_expvars)){
  ...
  if (i==1){all_res_suvr <-rbind(temp.mod.SUVR.crude, temp.mod.SUVR.adj1)} else {
    all_res_suvr <-rbind(all_res_suvr, temp.mod.SUVR.crude, temp.mod.SUVR.adj1)
  }
  
}

save(all_res_mri, file=".../all_res_mripet_v3.Rdata")
save(all_res_mri_over65, file=".../all_res_mripet_over65_v3.Rdata")
save(all_res_mri_petsamp, file=".../all_res_mri_petsamp_v3.Rdata")
save(all_res_pet, file=".../all_res_pet_v3.Rdata")
save(all_res_suvr, file=".../all_res_suvr_v3.Rdata")
