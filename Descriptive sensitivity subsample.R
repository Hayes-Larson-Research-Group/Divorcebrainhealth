...
load(file = ".../divorce_petmri.Rdata")
output_path <- "/.../"
...
write.xlsx(tab1_df,
           paste0(output_path, "table1_allsamp_petmri_v2.xlsx")
)

all_dat_final_df_complete <- all_dat_final_df2[complete.cases(all_dat_final_df2[ , c("STUDY", "W1_INTERVIEW_AGE", "W1_FEMALE", "W1_D_RACE_SUMMARY", "W1_USBORN", 
...
all_dat_final_df_complete <- all_dat_final_df_complete[!apply(all_dat_final_df_complete[, c("Total_wmh_log")], 1, function(x) any(is.infinite(x))), ]
...
write.xlsx(tab1_df_complete,
  paste0(output_path, "table1_allsamp_completecases_petmri_v2.xlsx")
)

save(all_dat_final_df_complete, file=".../divorce_completecases_petmri.Rdata")
